...
from video_audio_transcriber.transcriber import MissingAPIKeyError, MiniMaxTranscriber


class TestMiniMaxTranscriber:
    def test_transcribe_missing_api_key(self, tmp_path):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("MINIMAX_ASR_KEY", None)

            transcriber = MiniMaxTranscriber()
            audio_path = tmp_path / "test.mp3"
            audio_path.write_bytes(b"fake audio content")

            with pytest.raises(MissingAPIKeyError, match="MINIMAX_ASR_KEY"):
